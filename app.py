...
with st.container():
    ...
    if not contracts_df.empty:
        for idx, row in contracts_df.iterrows():
            # 날짜 비교 로직 (계약 만료 자동 판별)
            end_date = pd.to_datetime(row.get('end_date', '1900-01-01'))
            ...
            with st.container(border=True):
                ...
                # 금액 콤마 포맷팅 적용
                dep = format_currency(row.get('deposit_amount', 0))
                rent = format_currency(row.get('monthly_rent', 0))
                
                st.markdown(f"💰 **보증금**: {dep}원 | 💵 **월세**: {rent}원")
                st.markdown(f"🗓️ **계약 기간**: {row.get('start_date')} ~ {row.get('end_date')}")

    # 신규 등록 폼
    with st.expander("➕ 신규 계약 추가"):
        with st.form("new_contract", clear_on_submit=True):
            b_name = st.text_input("건물명")
            r_num = st.text_input("호실")
            dep = st.text_input("보증금(숫자만)")
            rent = st.text_input("월세(숫자만)")
            start = st.date_input("시작일")
            end = st.date_input("만료일")
            if st.form_submit_button("저장"):
                supabase.table("contracts").insert({
                    "building_name": b_name, "room_number": r_num,
                    "deposit_amount": re.sub(r'[^\d]', '', dep),
                    "monthly_rent": re.sub(r'[^\d]', '', rent),
                    "start_date": str(start), "end_date": str(end), "status": "계약중"
                }).execute()
                st.rerun()
