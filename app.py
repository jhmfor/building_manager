...
with tab2:
    ...
    total_cost = 0
    if not display_expenses.empty and "금액" in display_expenses.columns:
        numeric_costs = display_expenses["금액"].astype(str).str.replace(r'[^\d]', '', regex=True)
    ...
    if not display_expenses.empty:
        ...
        for idx, row in display_expenses.iterrows():
            ...
            with st.expander(f"🏢 {row.get('건물명', '')} | 📅 {row.get('날짜', '')} | 📝 {row.get('내용', '')} ({row.get('금액', '')}원)"):
                if row_id is not None:
                    with st.form(f"edit_expense_{row_id}_{idx}"):
                        ...
                        ed_cat = st.text_input("카테고리", value=str(row.get('카테고리', '')))
                        ed_desc = st.text_input("내용", value=str(row.get('내용', '')))
                        ed_amount = st.text_input("금액", value=str(row.get('금액', '')))
    ...
    with st.form("expense_add_form", clear_on_submit=True):
        col1, col2 = st.columns(2)
        ex_date = col1.date_input("지출 날짜", value=datetime.today())
        ex_bname = col2.text_input("건물명", placeholder="예: 부산 센토빌")
        
        col3, col4, col5 = st.columns(3)
        ex_cat = col3.text_input("카테고리", placeholder="예: 수리비")
        ex_desc = col4.text_input("지출 내용", placeholder="예: 도어락 교체")
        ex_amount = col5.text_input("지출 금액(원)", value="100,000")
